 ...
 */
export async function getOrCreateVector(word: string, embedding?: number[]): Promise<Vector> {
 ...
 */
export async function createVectorMazeSession(gameData: any): Promise<GameSession> {
 ...
 */
export async function getVectorMazeSession(gameId: string): Promise<GameSession | null> {
  const { data, error } = await supabase
    .from('game_sessions')
    .select('*')
    .eq('id', gameId)
 ...
 */
export async function updateVectorMazeSession(gameId: string, sessionData: any, score?: number): Promise<GameSession> {
 ...
 */
export async function getCachedSimilarity(word1: string, word2: string): Promise<number | null> {
  const { data, error } = await supabase
    .from('word_similarities')
    .select('similarity_score')
    .or(`and(word1.eq.${word1},word2.eq.${word2}),and(word1.eq.${word2},word2.eq.${word1})`)
    .single();
