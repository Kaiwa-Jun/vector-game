 ...
 */
export function getDifficultySettings(difficulty: 'easy' | 'medium' | 'hard'): DifficultySettings {
  switch (difficulty) {
    case 'easy':
      return {
 ...
 */
export async function generateWordPair(difficulty: 'easy' | 'medium' | 'hard'): Promise<WordPair> {
 ...
 */
export function isGoalReached(similarity: number, targetSimilarity: number): boolean {
 ...
 */
export function calculateFinalScore(gameData: VectorMazeGameData, isSuccess: boolean): ScoreResult {
  if (!isSuccess) {
    return {
      score: 0,
      isSuccess: false
